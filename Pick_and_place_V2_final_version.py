...
robot_path = os.path.join("..", "models", "descriptions", "robots", "arms","franka_description", "urdf", "panda_arm_hand.urdf")
...
class MotionProfile(LeafSystem):
    def __init__(self,update_period=1/50 ):
        ...
        self.index_q_next = 0
        self.flag = True
        self.min_distance = 0.025 # threshold for the colision avoidance
...
######################################################################################################
#                     ########Define TAMP sequencer as a LeafSystem #######   
######################################################################################################
...
# function to make a panda model for the Ik 
def make_panda_ik(panda_path, time_step):
    # Create the multibody system (robot model)
    plant = MultibodyPlant(time_step)

    # Load the Panda robot model into the plant
    parser = Parser(plant)
    parser.AddModelsFromUrl("file://" + os.path.abspath(panda_path))

    # Fix the robot base to the world frame
    base = plant.GetBodyByName("panda_link0")
    plant.WeldFrames(plant.world_frame(), base.body_frame())

    # Lock the model before using it (IK, dynamics, etc.)
    plant.Finalize()

    return plant

# solve Ik as an optimisation problem
...
# algorithm to generate a coherant sequence of pick and place sequence
...
def create_sim_scene(sim_time_step):   
   ...
   # Add TAMPSequencer and Pathplanner to controller  regulate the robot  
    sequencer = builder.AddSystem(TAMPSequencer(panda_ik=panda_ik,plant=plant,desired_order=order,X_WB_target=X_WB_circle,cube_names=cubes))
...
def run_simulation(sim_time_step):
    ...
    simulator.AdvanceTo(150.0)  # Adjust this time as needed
